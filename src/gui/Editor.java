...
public class Editor extends JDialog implements MouseListener{
	...
	private void agregarCasillas() {
		...
		for (int i = 0; i < filas; i++) {
			for (int j = 0; j < columnas; j++) {
				...
				celdas[i][j] = new Celda(numeroNodo);
				...
				final int columna = j;
					...
					if(grafo!=null){
						if((i+1<filas) && actual.getVecinos().contains(new Nodo( (i+1)*columnas+j  ))){
							...
							celdas[i][j].setVecinoInf(true);
						}
						if((i-1>=0) && actual.getVecinos().contains(new Nodo( (i-1)*columnas+j  ))){
							celdas[i][j].setVecinoSup(true);
						}
					}
					abajo.setPreferredSize(new Dimension(ancho_casilla,ancho_alto_boton));
					abajo.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							boolean vecinoInf = celdas[fila][columna].tieneVecinoInf();
							celdas[fila][columna].setVecinoInf(!vecinoInf);
							if (fila + 1 < filas) {
								celdas[fila + 1][columna].setVecinoSup(!vecinoInf);
							}
					...
					if(grafo!=null){
						if((j+1<columnas) && actual.getVecinos().contains(new Nodo( (i*columnas)+j+1 ))){
							...
							celdas[i][j].setVecinoDer(true);
						}
						if((j-1>=0) && actual.getVecinos().contains(new Nodo( (i*columnas)+j-1 ))){
							celdas[i][j].setVecinoIzq(true);
						}
					}
					derecha.setPreferredSize(new Dimension(ancho_alto_boton,alto_casilla));
					derecha.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							boolean vecinoDer = celdas[fila][columna].tieneVecinoDer();
							celdas[fila][columna].setVecinoDer(!vecinoDer);
							if (columna + 1 < columnas) {
								celdas[fila][columna + 1].setVecinoIzq(!vecinoDer);
							}
							JButton b = (JButton) e.getSource();
	...
	private void guardar() {
		...
		if (opcion == JFileChooser.APPROVE_OPTION) {
			...
			Util.guardar(archivo, celdas, inicio, fin);
